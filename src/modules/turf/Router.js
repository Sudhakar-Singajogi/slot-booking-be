...
const {
  ...
  turfExistsSchema,
} = require(path.resolve("src/modules/turf/Schema"));
// router.post("/byareana", checkArenaExists(), async (req, res, next) => {
router.post(
  "/byareana",
  checkArenaExists(),
  joiMiddleware(getTurfsSchema, "getTurfs"),
  async (req, res, next) => {
    //   let resultSet = {
...
router.post(
  "/sports",
  checkArenaExists(),
  joiMiddleware(getTurfsSportsSchema, "getTurfsSports"),
  async (req, res, next) => {
    const reqObj = req.body;
...
router.post(
  "/exists",
  checkArenaExists(),
  joiMiddleware(turfExistsSchema, "turfExists"),
  async (req, res, next) => {
    const reqObj = req.body;
